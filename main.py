...
# REMOVED: db_lock = asyncio.Lock()  # <-- REMOVE THIS LINE
...
class Database:
    ...
    async def update_user_interaction(self, user_id: int, username: str = None, 
        ...
        async with self.get_connection():
            ...
            if exists:
                # Update existing user - FIXED: removed duplicate first_name handling
                await self.execute_and_commit('''
                    UPDATE users 
...
async def cleanup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await self.execute_and_commit('''
        DELETE FROM files 
